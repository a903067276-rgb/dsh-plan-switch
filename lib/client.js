      ...
      ctx.slots.inject("conversation.input.left", () => ctx.slots.register(
        ...
        (props) => {
          ...
          const toggle = () => {
            ...
            // rc.6/rc.7 只收 2 参——先按 3 参调，报"参数个数"错则降级 2 参重试（版本自适应）
            ctx.remote.commands.execute(sessionId, "/plan", []).catch((error) => {
              const msg = error instanceof Error ? error.message : String(error);
              if (/business argument|expected .*argument|got \d+/i.test(msg)) {
                return ctx.remote.commands.execute(sessionId, "/plan");
              }
              throw error;
            }).then((result) => {
              if (!result || !result.ok) {
