  ...
  factory: (require) => {
    ...
    if (typeof document !== "undefined" && !document.getElementById("dsh-plan-switch-style")) {
      ...
      style.textContent = [
        ...
        // plan 开启：黄色系高亮（沿用官方 warn 语义，与旧版一致）
        ".dsh-plan-switch-btn.is-active{background:var(--dsw-alias-state-warn-tertiary,rgba(255,180,0,0.15));color:var(--dsw-alias-state-warn-label,#b8860b);}",
        // 命令失败：错误色
    ...
    function apply(ctx) {
      ctx.slots.inject("conversation.input.left", () => ctx.slots.register(
        ...
        (props) => {
          ...
          const title = notice
            ? notice
            : (active ? "退出 plan 模式（等同 /plan off）" : "进入 plan 模式（等同 /plan）");
